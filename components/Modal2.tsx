        ...
        >
            ...
            <motion.div key="modal" initial="hidden" animate="visible" exit="exit" variants={modal} className="fixed inset-0 flex flex-auto items-center justify-center p-4 py-20 top-0">
                ...
                <Dialog.Panel className="flow-root w-full max-w-5xl rounded-3xl bg-darkpurple1/50 text-text1 p-10 h-full overflow-y-auto">
                    <button className="float-right text-turquoise1 hover:text-yellow1 rounded-lg focus-visible:outline-none focus-visible:ring ring-offset-navy1 ring-offset-0 ring-red1 transition-all" onClick={() => { router.push('/projects'); }}><RiCloseCircleFill size={20} /></button>
                    <Dialog.Title className="mb-1 text-3xl font-bold">{title}</Dialog.Title>
                    <Dialog.Description className="text-lg">{description}</Dialog.Description>
                    <br />
                    {children}
